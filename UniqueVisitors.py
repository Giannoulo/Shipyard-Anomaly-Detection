...
ais_data = spark.read.format("csv").option("header", "true").option("inferSchema", "true").load('hdfs://Master1/user/nari_dynamic.csv')

#Read the ports csv, turn it to a list of tuples and broadcast it to the workers
ports_df = spark.read.format("csv").option("header", "true").option("inferSchema", "true").load('hdfs://Master1/user/ports.csv')
...
#Save to a text file
unique_visitors.coalesce(1).saveAsTextFile('~/Downloads/unique_visitors.txt')
